...
from runtime.network.tcp_relay import (
    PrivateEndpoint,
    RelayConfig,
    run_relay_once,
)
...
class TcpRelayTests(unittest.TestCase):
    ...
    def test_delay_is_pipelined_not_payload_logged(self):
        target_port, target_thread, target_errors = start_echo_server()
        ready = queue.Queue()
        holder = {}
        config = RelayConfig(
        ...
        relay_thread = threading.Thread(
            target=lambda: holder.setdefault(
                "metrics",
                run_relay_once(config, on_ready=lambda endpoint: ready.put(endpoint)),
            ),
            daemon=True,
        )
        relay_thread.start()
    ...
    def test_disconnect_after_seconds_is_recorded(self):
        ...
        try:
            ...
            target_thread.join(timeout=5)
        self.assertFalse(relay_thread.is_alive())
        self.assertEqual(holder["metrics"].termination["reason"], "disconnect_after_seconds")
        self.assertFalse(target_errors)
