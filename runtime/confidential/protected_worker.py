...
class ProtectedWorkerSessionManager:
    """One-shot, attestation-bound worker sessions with durable replay tombstones."""
    ...
    def execute(self, envelope: ConfidentialEnvelope | Mapping[str, Any]) -> ProtectedWorkerResult:
        ...
        plaintext = self._decrypt_json(parsed, state)
        request = plaintext
    ...
    def _claim(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise ProtectedWorkerError("protected session not found")
            instant = (now or datetime.now(UTC)).astimezone(UTC)
