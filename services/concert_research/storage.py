...
from .classifier import classify_event, EventClassification
...
logger = logging.getLogger("concert_storage")


class ConcertStore:
    """SQLite/WAL reference store for sources, frontier, pages, venues and events."""
    ...
    def _init(self) -> None:
        with self.connection() as con:
            ...
            CREATE TABLE IF NOT EXISTS venues(
              ...
              address TEXT,venue_types_json TEXT NOT NULL DEFAULT '[]',genres_json TEXT NOT NULL DEFAULT '[]',
              official_url TEXT,event_calendar_url TEXT,source_confidence REAL NOT NULL DEFAULT 0.8,
              UNIQUE(canonical_name,city_key));
            ...
            CREATE TABLE IF NOT EXISTS events(
              ...
              is_live_music INTEGER NOT NULL DEFAULT 0,
              live_music_confidence REAL NOT NULL DEFAULT 0.0,
              genres_json TEXT NOT NULL DEFAULT '[]',
              genre_confidence REAL NOT NULL DEFAULT 0.0,
              artist_names_json TEXT NOT NULL DEFAULT '[]',
              headliner TEXT,
              support_acts_json TEXT NOT NULL DEFAULT '[]',
              verification_status TEXT NOT NULL DEFAULT 'probable',
              verification_score REAL NOT NULL DEFAULT 0.8,
              quality_score REAL NOT NULL DEFAULT 50.0,
              recommendation_score REAL NOT NULL DEFAULT 50.0,
    ...
    def _migrate(self) -> None:
        """Non-destructively alters tables if upgraded from earlier schema versions."""
        with self.connection() as con:
            # 1. Migrate events table
            event_cols = {row["name"] for row in con.execute("PRAGMA table_info(events)").fetchall()}
            migrations = [
                ("primary_category", "TEXT NOT NULL DEFAULT 'other'"),
                ("secondary_categories_json", "TEXT NOT NULL DEFAULT '[]'"),
                ("category_confidence", "REAL NOT NULL DEFAULT 0.5"),
            ...
            for col_name, col_def in migrations:
                if col_name not in event_cols:
                    con.execute(f"ALTER TABLE events ADD COLUMN {col_name} {col_def}")

            # 2. Migrate venues table
            venue_cols = {row["name"] for row in con.execute("PRAGMA table_info(venues)").fetchall()}
            venue_migrations = [
            ...
            for col_name, col_def in venue_migrations:
                if col_name not in venue_cols:
                    con.execute(f"ALTER TABLE venues ADD COLUMN {col_name} {col_def}")

            # 3. Create missing indexes
            con.execute("CREATE INDEX IF NOT EXISTS idx_events_category ON events(primary_category)")
            con.execute("CREATE INDEX IF NOT EXISTS idx_events_live_music ON events(is_live_music)")
            con.execute("CREATE INDEX IF NOT EXISTS idx_events_date_cat ON events(date_iso, primary_category)")
    ...
    def upsert_source(self, source: SourceRecord) -> tuple[int, bool]:
        ...
        with self.connection() as con:
            ...
            con.execute(
                """INSERT INTO sources(canonical_url,name,tier,source_type,city_key,priority,score,first_seen,last_seen,last_crawled,next_crawl,consecutive_failures,active)
              VALUES(?,?,?,?,?,?,?,?,?,?,?,?,1)
        ...
        *,
        ...
        with self.connection() as con:
            ...
            if not row:
                return
            con.execute(
                "UPDATE sources SET source_type=?,tier=?,score=?,priority=? WHERE source_id=?",
                (
                    source_type or row["source_type"],
                    tier if tier in {"primary", "secondary"} else row["tier"],
                    max(-100.0, min(1000.0, float(row["score"]) + score_delta)),
                    int(priority) if priority is not None else int(row["priority"]),
                    source_id,
                ),
            )
    ...
    def record_source_failure(self, source_id: int) -> None:
        with self.connection() as con:
            con.execute(
                "UPDATE sources SET consecutive_failures=consecutive_failures+1,score=MAX(-100,score-2) WHERE source_id=?",
                (source_id,),
            )
    ...
    def enqueue(
        ...
        *,
        ...
        with self.connection() as con:
            ...
            con.execute(
                """INSERT INTO frontier(url,source_id,city_key,depth,priority,discovered_from,first_seen,next_attempt,state)
    ...
    def due_frontier(self, limit: int) -> list[sqlite3.Row]:
        with self.connection() as con:
            return list(
                con.execute(
                    """SELECT f.*,s.name source_name,s.source_type,s.tier,s.score,s.priority source_priority,s.etag,s.last_modified
    ...
    def mark_frontier(self, url: str, state: str, *, delay_minutes: int = 0, failure: bool = False) -> None:
        ...
        with self.connection() as con:
            con.execute(
                "UPDATE frontier SET state=?,last_attempt=?,next_attempt=?,failures=failures+? WHERE url=?",
    ...
    def source_for_url(self, url: str) -> sqlite3.Row | None:
        ...
        with self.connection() as con:
            ...
            return row or con.execute(
                "SELECT * FROM sources WHERE canonical_url LIKE ? ORDER BY priority LIMIT 1",
                (f"%://{host}/%",),
            ).fetchone()
            ...
            con.execute(
              ...
              extraction_count=excluded.extraction_count""",
                (
                    ...
                    last_modified,
                    title[:500],
                    meaningful_text[:4000],
                    extraction_count,
                ),
            )
            if source_id:
                con.execute(
                    """UPDATE sources SET last_crawled=?,etag=COALESCE(?,etag),last_modified=COALESCE(?,last_modified),
    ...
    def _venue_id(
        ...
        if row:
            vid = int(row[0])
            con.execute(
                "UPDATE venues SET latitude=COALESCE(latitude,?),longitude=COALESCE(longitude,?),address=COALESCE(address,?) WHERE venue_id=?",
                (lat, lon, area, vid),
            )
        else:
            vid = int(
                con.execute(
                    "INSERT INTO venues(canonical_name,city_key,latitude,longitude,address) VALUES(?,?,?,?,?)",
                    (canonical, city_key, lat, lon, area),
                ).lastrowid
            )
    ...
    def upsert_event(
        ...
        *,
        ...
        cls: EventClassification = classify_event(
            title=obs.title,
            venue=obs.venue,
            description=obs.description,
            area=obs.area,
            genre_hint=obs.genre,
            artist_info=obs.artist_info,
            source_name=obs.source_name,
            price=obs.price,
            url=obs.event_url or obs.source_url,
        )
        ...
        with self.connection() as con:
            ...
            if existing is None:
                candidates = con.execute(
                    """SELECT e.*,v.canonical_name venue_norm FROM events e JOIN venues v ON v.venue_id=e.venue_id
                  WHERE e.date_iso=? AND (e.start_time IS NULL OR ? IS NULL OR e.start_time=?) LIMIT 50""",
                    (obs.date_iso, obs.start_time, obs.start_time),
                ).fetchall()
                best = None
                for cand in candidates:
                    ...
                    if ts >= 0.84 and vs >= 0.78 and (best is None or ts * 0.7 + vs * 0.3 > best[0]):
                        best = (ts * 0.7 + vs * 0.3, cand)
                if best:
                    event_id, existing = best[1]["event_id"], best[1]

            genre_str = obs.genre or (", ".join(cls.genres) if cls.genres else None)

            con.execute(
                """INSERT INTO events(
                  event_id,canonical_title,venue_id,date_iso,start_time,end_time,price,description,genre,artist_info,status,
                  area,event_url,confidence,first_seen,last_seen,last_verified,observation_count,
                  primary_category,secondary_categories_json,category_confidence,is_live_music,live_music_confidence,
                  genres_json,genre_confidence,artist_names_json,headliner,support_acts_json,verification_status,
                  verification_score,quality_score,recommendation_score,classification_version,classified_at
                ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ON CONFLICT(event_id) DO UPDATE SET
                  ...
                  category_confidence=MAX(events.category_confidence,excluded.category_confidence),
                  is_live_music=MAX(events.is_live_music,excluded.is_live_music),
                  live_music_confidence=MAX(events.live_music_confidence,excluded.live_music_confidence),
                  genres_json=excluded.genres_json,
                  genre_confidence=MAX(events.genre_confidence,excluded.genre_confidence),
                  artist_names_json=excluded.artist_names_json,
                  headliner=COALESCE(excluded.headliner,events.headliner),
                  support_acts_json=excluded.support_acts_json,
                  verification_status=excluded.verification_status,
                  verification_score=MAX(events.verification_score,excluded.verification_score),
                ...
                (
                    ...
                    json.dumps(cls.secondary_categories, ensure_ascii=False),
                    cls.category_confidence,
                    1 if cls.is_live_music else 0,
                    cls.live_music_confidence,
                    json.dumps(cls.genres, ensure_ascii=False),
                    cls.genre_confidence,
                    json.dumps(cls.artist_names, ensure_ascii=False),
                    cls.headliner,
                    json.dumps(cls.support_acts, ensure_ascii=False),
                    cls.verification_status,
                    cls.verification_score,
                    cls.quality_score,
                    cls.recommendation_score,
                    1,
                    now,
                ),
            )
            con.execute(
                """INSERT OR IGNORE INTO event_observations(event_id,source_id,observed_at,raw_json,fingerprint,extraction_method,confidence)
              VALUES(?,?,?,?,?,?,?)""",
                (
                    event_id,
                    source_id,
            ...
            if source_id:
                con.execute(
                    """INSERT INTO event_sources VALUES(?,?,?,?,?) ON CONFLICT(event_id,source_id,event_url)
    ...
    def reclassify_all_events(self, limit: int = 50000) -> int:
        """Backfills and updates classification across all existing events in the database."""
        now = utc_now_iso()
        with self.connection() as con:
            rows = con.execute(
                """SELECT e.event_id, e.canonical_title, e.description, e.genre, e.artist_info, e.area, e.price, e.event_url,
                          v.canonical_name venue_name
                   FROM events e JOIN venues v ON v.venue_id=e.venue_id LIMIT ?""",
                (limit,),
            ).fetchall()
            ...
            for row in rows:
                cls: EventClassification = classify_event(
                    ...
                    area=row["area"],
                    genre_hint=row["genre"],
                    artist_info=row["artist_info"],
                    price=row["price"],
                    url=row["event_url"] or "",
                )
                con.execute(
                    """UPDATE events SET
                        primary_category=?,
    ...
    def research_rows(
        ...
        *,
        ...
        """Queries events matching spatial, temporal, category and genre filters."""
        city_key = self.city_key(city)
        params: list[Any] = [date_from, date_to]

        where_clauses = ["e.date_iso BETWEEN ? AND ?"]

        # Category filter
        if categories:
            cat_placeholders = ",".join("?" for _ in categories)
            where_clauses.append(
                f"(e.primary_category IN ({cat_placeholders}) OR EXISTS (SELECT 1 FROM json_each(e.secondary_categories_json) WHERE value IN ({cat_placeholders})))"
            )
        ...
        # Live-music filter
        if only_live_music:
            where_clauses.append("(e.primary_category = 'concert' OR e.is_live_music = 1)")

        # Spatial / city bounds
        has_coords = latitude is not None and longitude is not None and radius_km and radius_km > 0
        if has_coords:
            import math
            ...
            where_clauses.append(spatial_sql)
            params.extend([city_key, min_lat, max_lat, min_lon, max_lon, min_lat, max_lat, min_lon, max_lon])
        else:
            where_clauses.append("v.city_key = ?")
            params.append(city_key)

        # Sorting logic
        if sort == "quality":
            order_sql = "e.date_iso, e.quality_score DESC, e.confidence DESC"
        elif sort == "recommended":
            order_sql = "e.date_iso, e.recommendation_score DESC, e.confidence DESC"
        else:  # sort == "date"
            order_sql = "e.date_iso, COALESCE(e.start_time, '99:99'), e.confidence DESC"

        params.append(max_events * 2 if genres else max_events)

        where_combined = " AND ".join(where_clauses)
        query = f"""SELECT e.*,v.latitude venue_latitude,v.longitude venue_longitude,v.city_key venue_city_key,
        ...
        # In-memory genre filtering if specified
        if genres:
            target_genres = {g.strip().lower() for g in genres if g.strip()}
            filtered = []
            for r in rows:
                ev_genres = set()
                if r["genre"]:
                    ev_genres.update(g.strip().lower() for g in r["genre"].split(","))
                if r["genres_json"]:
                    try:
                        ev_genres.update(g.strip().lower() for g in json.loads(r["genres_json"]))
                    except Exception:
                        pass
                if ev_genres & target_genres:
                    filtered.append(r)
            return filtered[:max_events]

        return list(rows[:max_events])

    def sources_for_city(self, city: str) -> list[sqlite3.Row]:
        with self.connection() as con:
            return list(
                con.execute(
                    "SELECT * FROM sources WHERE city_key=? AND active=1 ORDER BY CASE tier WHEN 'primary' THEN 0 ELSE 1 END,priority,score DESC",
                    (self.city_key(city),),
                )
            )
    ...
    def record_query_stats(
        ...
        *,
        ...
        reward = new_domains * 4.0 + new_events * 6.0 + min(results, 10) * 0.2 - irrelevant * 0.5
        with self.connection() as con:
            con.execute(
                """INSERT INTO query_stats VALUES(?,?,?,1,?,?,?,?,?) ON CONFLICT(query) DO UPDATE SET last_run=excluded.last_run,
              ...
              score=query_stats.score*.8+excluded.score*.2""",
                (query, family, utc_now_iso(), results, new_domains, new_events, irrelevant, reward),
            )
