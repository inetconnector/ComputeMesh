...
benchmark, a model manifest and an explicit layer count. Until a correct shared
runtime result exists it never predicts or claims shared latency/speedup.
"""
...
def _same_benchmark_model(artifact: dict[str, Any], labelled: list[tuple[str, dict[str, Any]]]) -> tuple[str, int]:
    ...
    for label, result in labelled:
        ...
        if isinstance(measured_size, bool) or not isinstance(measured_size, (int, float)) or int(measured_size) != size:
...
def _local_candidate(node: dict[str, Any], model_size: int, layer_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "feasible": feasible,
        "layer_ranges": [{
            "node_id": node["node_id"], "start_layer": 0, "end_layer_exclusive": layer_count
        }] if feasible else [],
        "tensor_split": [1.0] if feasible else [],
        "explanation": (
            "selected coordinator memory budget can hold the complete artifact"
            if feasible else "selected coordinator memory budget cannot hold the complete artifact"
        ),
    }


def _shared_candidate(
    coordinator: dict[str, Any], worker: dict[str, Any], model_size: int, layer_count: int, overhead_fraction: float
) -> dict[str, Any]:
    fixed = math.ceil(model_size * overhead_fraction)
    per_layer = max(1, math.ceil(max(1, model_size - fixed) / layer_count))
    coord_max = min(layer_count, max(0, coordinator["usable_memory_bytes"] - fixed) // per_layer)
    ...
    if not feasible:
        return {
            "mode": "shared_contiguous_layers", "feasible": False, "layer_ranges": [], "tensor_split": [],
    ...
    return {
        ...
        "feasible": True,
        "layer_ranges": [
            {"node_id": coordinator["node_id"], "start_layer": 0, "end_layer_exclusive": int(coord_layers)},
            {"node_id": worker["node_id"], "start_layer": int(coord_layers), "end_layer_exclusive": layer_count},
        ],
...
def build_placement_decision(
    *,
    ...
    _require_benchmark(coordinator_prefill, "llama_cpp_prefill", coordinator_profile, "coordinator prefill")
    _require_benchmark(coordinator_decode, "llama_cpp_decode", coordinator_profile, "coordinator decode")
    ...
    model_name, model_size = _same_benchmark_model(artifact, [
        ("coordinator prefill", coordinator_prefill), ("coordinator decode", coordinator_decode),
        ("worker prefill", worker_prefill), ("worker decode", worker_decode),
    ])
    ...
    coord_fresh = coord_age <= policy.max_profile_age_hours and coord_future <= policy.max_future_skew_minutes
    worker_fresh = worker_age <= policy.max_profile_age_hours and worker_future <= policy.max_future_skew_minutes
    constraints = [
        _constraint("coordinator_not_draining", coord_not_draining, "provider_limits.draining must be false"),
        _constraint("worker_not_draining", worker_not_draining, "provider_limits.draining must be false for shared placement"),
        _constraint("coordinator_profile_fresh", coord_fresh, f"age_hours={coord_age:.3f}; future_skew_minutes={coord_future:.3f}"),
        _constraint("worker_profile_fresh", worker_fresh, f"age_hours={worker_age:.3f}; future_skew_minutes={worker_future:.3f}"),
        _constraint("network_peer_assertion_matches_worker", True, "peer binding is caller-asserted because benchmark_result v1 lacks target node_id"),
    ]

    local = _local_candidate(coordinator, model_size, layer_count)
    shared = _shared_candidate(coordinator, worker, model_size, layer_count, policy.fixed_model_overhead_fraction)
    ...
    if shared["feasible"]:
        ...
        explanation = "shared placement is memory-feasible; execute it only as an experiment and compare with local baseline before any production ranking"
    elif local["feasible"]:
        mode = "local_only"
        explanation = "shared placement is unavailable under current hard/memory constraints; coordinator local baseline remains feasible"
    else:
        mode = "no_plan"
    ...
    performance = {
        ...
        "coordinator_prefill_tokens_per_second": _positive_metric(coordinator_prefill, "prefill_tokens_per_second_avg"),
        "coordinator_decode_tokens_per_second": _positive_metric(coordinator_decode, "decode_tokens_per_second_avg"),
        "worker_prefill_tokens_per_second": _positive_metric(worker_prefill, "prefill_tokens_per_second_avg"),
        "worker_decode_tokens_per_second": _positive_metric(worker_decode, "decode_tokens_per_second_avg"),
        ...
        "network_download_mbps_p50": _positive_metric(network_result, "download_mbps_p50"),
        "predicted_shared_request_ms": None,
        "predicted_speedup_vs_local": None,
        "reason": "individual compute/path benchmarks do not determine shared llama.cpp cost without a correct measured shared run and transfer trace",
    }

    identity = {
        "model_digest": artifact["digest"], "layer_count": layer_count,
        "coordinator_node_id": coordinator_profile["node_id"], "coordinator_revision": coordinator_profile["profile_revision"],
        "worker_node_id": worker_profile["node_id"], "worker_revision": worker_profile["profile_revision"],
        "benchmark_run_ids": [coordinator_prefill["run_id"], coordinator_decode["run_id"], worker_prefill["run_id"], worker_decode["run_id"], network_result["run_id"]],
        "policy": {
    ...
    decision = {
        ...
        "model": {
            "model_id": model_manifest["model_id"], "model_version": model_manifest["model_version"],
            "artifact_digest": artifact["digest"], "artifact_size_bytes": model_size,
            "benchmark_model_name": model_name, "layer_count": layer_count,
        },
        "nodes": {
            "coordinator": {**coordinator, "profile_revision": coordinator_profile["profile_revision"], "profile_age_hours": round(coord_age, 6)},
            "worker": {**worker, "profile_revision": worker_profile["profile_revision"], "profile_age_hours": round(worker_age, 6)},
        },
        "network_evidence": {"run_id": network_result["run_id"], "peer_node_id": network_peer_node_id, "peer_binding": "caller_asserted_v1"},
        "hard_constraints": constraints,
        "candidates": [local, shared],
        "performance_evidence": performance,
        "recommendation": {"mode": mode, "production_scheduling": False, "explanation": explanation},
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    for option in ("coordinator-profile", "worker-profile", "model-manifest", "coordinator-prefill", "coordinator-decode", "worker-prefill", "worker-decode", "network"):
    ...
    try:
        decision = build_placement_decision(
            coordinator_profile=_read_json(args.coordinator_profile), worker_profile=_read_json(args.worker_profile),
            model_manifest=_read_json(args.model_manifest), coordinator_prefill=_read_json(args.coordinator_prefill),
            coordinator_decode=_read_json(args.coordinator_decode), worker_prefill=_read_json(args.worker_prefill),
            worker_decode=_read_json(args.worker_decode), network_result=_read_json(args.network),
            network_peer_node_id=args.network_peer_node_id, layer_count=args.layer_count,
            artifact_digest=args.artifact_digest, policy=policy,
        )
