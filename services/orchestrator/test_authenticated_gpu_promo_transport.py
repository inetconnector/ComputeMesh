...
def test_gpu_promo_transport_fails_closed_without_negotiated_capability() -> None:
    ...
    with pytest.raises(AuthenticatedGpuPromoTransportError, match="did not negotiate"):
        transport.request_gpu_promo_challenge(
            node_id="node-a",
            challenge_document=_challenge(),
            timeout_seconds=30.0,
        )
...
def test_gpu_promo_transport_rejects_response_for_another_key() -> None:
    ...
    with pytest.raises(AuthenticatedGpuPromoTransportError, match="proof key id mismatch"):
        transport.request_gpu_promo_challenge(
            node_id="node-a",
            challenge_document=_challenge(),
            timeout_seconds=30.0,
