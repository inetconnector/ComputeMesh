...
class OwnerAccountStore:
    ...
    def _init_db(self) -> None:
        with self._connection() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS owner_mcp_settings (
                    owner_id TEXT PRIMARY KEY,
    ...
    # -- mcp tool settings per fleet / owner ----------------------------------
    ...
    def get_mcp_disabled_tools(self, owner_id: str) -> list[str]:
        ...
        with self._connection() as conn:
            row = conn.execute(
                "SELECT disabled_tools FROM owner_mcp_settings WHERE owner_id = ?",
                (cleaned_id,),
            ).fetchone()
            if not row:
                return []
    ...
    def set_mcp_disabled_tools(self, owner_id: str, disabled_tools: list[str]) -> None:
        ...
        with self._connection() as conn:
            conn.execute(
                """
                INSERT INTO owner_mcp_settings(owner_id, disabled_tools, updated_at)
                VALUES (?, ?, ?)
