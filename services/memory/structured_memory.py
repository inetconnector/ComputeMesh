...
class StructuredMemoryStore:
    ...
    def put(
        ...
        *,
        ...
        if scope_obj is not MemoryScope.GLOBAL and not clean_scope_id:
            raise ValueError("scope_id is required outside GLOBAL scope")
        confidence = min(1.0, max(0.0, float(confidence)))
        authority = min(1.0, max(0.0, float(authority)))
        now = time.time()
        record = MemoryRecord(
            memory_id=f"mem_{uuid.uuid4().hex}", key=clean_key, value=clean_value,
            memory_type=type_obj, scope=scope_obj, scope_id=clean_scope_id,
            provenance=str(provenance or "unknown"), confidence=confidence, authority=authority,
            explicit=bool(explicit), sensitivity=sensitivity_obj, consent=str(consent or ""),
            purpose=str(purpose or ""), valid_from=now, valid_until=valid_until,
            supersedes=supersedes, derived_from=tuple(str(x) for x in derived_from),
        ...
        with self._lock, self._conn:
            ...
            if supersedes:
                self._conn.execute(
                    "UPDATE memories SET deleted_at=?,updated_at=? WHERE memory_id=? AND deleted_at IS NULL",
                    (now, now, supersedes),
                )
        return record
    ...
    def suppress(self, key_pattern: str, *, scope: MemoryScope | str, scope_id: str, reason: str) -> str:
        ...
        with self._lock, self._conn:
            self._conn.execute(
                ...
                (sid, scope_obj.value, scope_id, self._normalize_key(key_pattern), str(reason), time.time()),
            )
        return sid
    ...
    def list_active(self, *, scope: MemoryScope | str, scope_id: str, include_global: bool = True, now: float | None = None) -> list[MemoryRecord]:
        scope_obj = self._coerce_enum(MemoryScope, scope)
        timestamp = time.time() if now is None else float(now)
        clauses = ["(scope=? AND scope_id=?)"]
        params: list[Any] = [scope_obj.value, scope_id]
        if include_global:
        ...
        suppressions = self._suppressed_patterns(scope_obj, scope_id)
        result = [self._row_to_record(row) for row in rows]
        return [r for r in result if not any(r.key == pattern or r.key.startswith(pattern + ".") for pattern in suppressions)]

    @staticmethod
    def _tokens(text: str) -> set[str]:
        return {tok for tok in re.findall(r"[\wäöüßÀ-ÿ.-]+", str(text).casefold()) if len(tok) > 1}

    def retrieve(self, query: str, *, scope: MemoryScope | str, scope_id: str, limit: int = 12, include_global: bool = True, now: float | None = None) -> list[MemoryRecord]:
        timestamp = time.time() if now is None else float(now)
        query_tokens = self._tokens(query)
        scored: list[tuple[float, MemoryRecord]] = []
        scope_obj = self._coerce_enum(MemoryScope, scope)
        for record in self.list_active(scope=scope_obj, scope_id=scope_id, include_global=include_global, now=timestamp):
            ...
            score = overlap * 0.48 + record.authority * 0.22 + record.confidence * 0.13 + recency * 0.05 + scope_bonus + negative_bonus
    ...
    def conflicts(self, key: str, *, scope: MemoryScope | str, scope_id: str) -> list[MemoryRecord]:
        clean_key = self._normalize_key(key)
        records = [record for record in self.list_active(scope=scope, scope_id=scope_id) if record.key == clean_key]
        values = {record.value for record in records}
        return records if len(values) > 1 else []

    def delete(self, memory_id: str, *, cascade_derived: bool = True) -> int:
        now = time.time()
        deleted = 0
        with self._lock, self._conn:
            ...
            if cascade_derived:
                rows = self._conn.execute("SELECT memory_id,derived_from_json FROM memories WHERE deleted_at IS NULL").fetchall()
                for row in rows:
                    deps = set(json.loads(row["derived_from_json"] or "[]"))
    ...
    def purge_expired(self, *, now: float | None = None) -> int:
        ...
        with self._lock, self._conn:
            cursor = self._conn.execute(
                "UPDATE memories SET deleted_at=?,updated_at=? WHERE deleted_at IS NULL AND valid_until IS NOT NULL AND valid_until<=?",
    ...
    def import_legacy_json(self, path: str | Path, *, scope_id: str, provenance: str = "legacy_user_memory_json") -> dict[str, Any]:
        source = Path(path)
        ...
        if existing:
            return {"status": "already_imported", "imported": int(existing["imported_count"]), "checksum": checksum}
        ...
        if profile.get("name"):
            self.put("profile.name", str(profile["name"]), memory_type=MemoryType.FACT, scope=MemoryScope.USER, scope_id=scope_id, provenance=provenance)
            imported += 1
        if profile.get("preferred_language"):
            self.put("profile.preferred_language", str(profile["preferred_language"]), memory_type=MemoryType.PREFERENCE, scope=MemoryScope.USER, scope_id=scope_id, provenance=provenance)
            imported += 1
        for index, pref in enumerate(profile.get("preferences") or []):
            self.put(f"profile.preference.{index}", str(pref), memory_type=MemoryType.PREFERENCE, scope=MemoryScope.USER, scope_id=scope_id, provenance=provenance)
            imported += 1
        for index, fact in enumerate(profile.get("facts") or []):
            self.put(f"profile.fact.{index}", str(fact), memory_type=MemoryType.FACT, scope=MemoryScope.USER, scope_id=scope_id, provenance=provenance)
            imported += 1
        ...
        for key, item in memories.items():
            ...
            self.put(
                str(key), str(item["value"]), memory_type=MemoryType.FACT,
                scope=MemoryScope.USER, scope_id=scope_id,
                provenance=f"{provenance}:{category}", explicit=True,
            )
            imported += 1
        with self._lock, self._conn:
            self._conn.execute(
    ...
    def health(self) -> dict[str, Any]:
        with self._lock:
            ...
            active = self._conn.execute("SELECT COUNT(*) FROM memories WHERE deleted_at IS NULL").fetchone()[0]
            deleted = self._conn.execute("SELECT COUNT(*) FROM memories WHERE deleted_at IS NOT NULL").fetchone()[0]
