...
**Last updated:** 2026-08-24 15:05 CEST
...
**Production services/runtime:** no production inference/control/payment runtime. The public web/update server is reachable by SSH as `supersrv-trixie` and runs active `computemesh-autoupdate.service` plus `computemesh-gateway.service`. Gateway billing now has a fail-closed real Stripe Checkout/Webhook implementation path plus a Stripe Connect Accounts v2 provider onboarding/settlement execution path. The webserver is configured with Stripe **testmode** keys, a test webhook endpoint, a Stripe Accounts v2 thin-event destination, durable test ledger/session paths, durable SQLite account/settlement store, and operator-controlled provider metering attribution to `node_test_settle_02`. A real Stripe test Checkout for the laptop user completed and was credited through the signed public webhook path after the SDK-object normalization fix. Stripe Connect is activated in the `InetConnector Sandbox` account for marketplace testing, but provider onboarding/KYC is now blocked on the operator's not-yet-founded German UG and the resulting legal/KYC/business data; live-mode Stripe credentials and production database/ops controls are not configured end-to-end for real customer funds.
**Release/version truth:** signed update release `1.2.9` is committed, tagged, pushed, deployed to the live webserver, and installed on the reachable LAN miner. Windows/Linux artifacts were rebuilt for the signed release, NodeOS ISO/IMG manifest entries were preserved, the webserver manifest signature verifies, and `http://192.168.1.27:8080/api/status` reports `software.current_version = "1.2.9"`. `main` now contains additional server-side Stripe billing fixes after the `v1.2.9` app artifact tag; no newer signed client artifact release has been cut yet. `server-stripe-connect-v2-20260824`, `server-stripe-v2-webhooks-20260824`, and `server-provider-shares-20260824` are non-release deployment tags for server-only billing activation. `v1.2.8` is the previous pushed signed release tag.
...
- canonical merged **code baseline before this work block**: `21bedc6` (`docs(state): record provider share deployment`)
...
- pushed `main` contains the `v1.2.9` release commit `e2612d2f01802527f5eb40569c88df57cb5c09dc` plus subsequent server-side Stripe testmode commits through `8d99404` (`feat(gateway): configure provider metering shares`); the webserver has fast-forward deployed this commit;
...
7. Verify the implemented Stripe Connect provider-settlement path against a real Stripe test connected account and then live Connect onboarding before production. Operator bank payout remains governed by the platform Stripe account's payout settings plus internal `revenue:network_fee` closing entries.
...
5. **Settlement executor:** `SettlementExecutor.run_provider_settlement(...)` requires a registered provider, a Stripe connected account, `payouts_enabled=True`, and a payable balance above `MINIMUM_PAYOUT_MICRO_UNITS`. It writes a pending settlement record, creates an idempotent Stripe Transfer using `computemesh:{settlement_id}`, and only then calls `ledger.create_provider_payout(..., settlement_reference=settlement_id)` to clear the internal provider payable.
...
1. Stripe test onboarding for provider `node_test_settle_02` / `acct_1U7twhDbl73CGVC7` reached the business-details stage after hCaptcha/phone verification and selecting company type `Kapitalgesellschaft` with structure `GmbH / Unternehmergesellschaft (UG)`. It cannot be completed honestly until the operator's UG is founded and the exact legal company name, register number, address, representative/KYC data, and payout bank details exist. After that, finish onboarding, refresh status until `payouts_enabled=true`, generate a provider payable of at least `MINIMUM_PAYOUT_MICRO_UNITS`, and run `POST /v1/admin/settlements/provider` in Stripe testmode.
...
6. As of 2026-08-24 15:05 CEST, SSH access to `supersrv-trixie` is verified, `computemesh-gateway.service` and `computemesh-autoupdate.service` are active, the server repository is clean at `21bedc6`, provider refresh returns `stripe_onboarding_status=requirements_past_due`, `payouts_enabled=false`, `details_submitted=false`, and provider payable is `6750` micro-units. Admin settlement correctly fails closed with HTTP 400 because Stripe payouts are not enabled. The live ledger reconciles as balanced with 6 transactions, `cust_laptop_frede=14991000`, `provider:node_test_settle_02=6750`, `revenue:network_fee=2250`, and `gateway:escrow=45000000` micro-units.
7. Local verification on 2026-08-24 15:05 CEST: `python -m unittest services.billing.tests.test_ledger services.billing.tests.test_accounting_and_settlement services.billing.tests.test_stripe_integration services.billing.tests.test_crypto_payments services.gateway.tests.test_gateway_server services.portal.tests.test_portal_server -v` ran 54 tests successfully.
