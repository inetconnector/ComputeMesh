...
**Last updated:** 2026-08-24 10:48 CEST
...
Verified on 2026-08-24 after the Stripe testmode Checkout/Webhook fixes:

- pushed `main` contains the `v1.2.9` release commit `e2612d2f01802527f5eb40569c88df57cb5c09dc` plus subsequent server-side Stripe testmode commits through `068cd88`;
...
4. **Stripe Connect service:** `services/billing/stripe_connect.py` adds `StripeConnectService` for Stripe Express connected-account creation, onboarding-link creation, connected-account status retrieval, and idempotent Transfers to connected accounts.
...
8. **Configuration:** `COMPUTEMESH_ACCOUNT_STORE_PATH` enables the SQLite operational store and webhook event inbox. With `STRIPE_API_KEY` configured, the same official Stripe SDK path is used for Checkout and Connect. Without the account store, provider registration/settlement endpoints fail closed.
...
2. `python -m unittest services.billing.tests.test_accounting_and_settlement services.billing.tests.test_stripe_integration services.gateway.tests.test_gateway_server -v` ran 25 tests successfully after adding Provider/Connect/Settlement tests.
3. `python -m unittest services.billing.tests.test_ledger services.billing.tests.test_accounting_and_settlement services.billing.tests.test_stripe_integration services.billing.tests.test_crypto_payments services.gateway.tests.test_gateway_server services.portal.tests.test_portal_server -v` ran 47 tests successfully.
4. `git diff --check` passed.
5. Commit `0f137c4` (`feat(billing): add stripe connect settlements`) was pushed to `origin/main` and fast-forward deployed to `/root/ComputeMesh` on `supersrv-trixie`; `computemesh-gateway.service` restarted successfully and `git rev-parse --short HEAD` returns `0f137c4`.
...
9. Stripe Connect external-account activation is still blocked in the Stripe account: calling `/v1/providers/stripe/onboarding` in testmode returned Stripe error `You can only create new accounts if you've signed up for Connect`. The code path fails closed correctly; a human must enable/sign up for Stripe Connect at `https://dashboard.stripe.com/connect` for this Stripe account before real or test connected-account onboarding links can be created.

### Remaining Before Real Money
1. Enable/sign up for Stripe Connect in the Stripe Dashboard for the current Stripe account, then create at least one Stripe Connect test provider through `/v1/providers/stripe/onboarding`, complete test onboarding in Stripe, refresh status, generate a real provider payable through metered usage, and run `POST /v1/admin/settlements/provider` in Stripe testmode.
...
5. Wire real scheduler/provider attribution into `record_job_execution(...)`; the gateway still meters demo completions against hardcoded provider `lab-mesh-default-rig`.
