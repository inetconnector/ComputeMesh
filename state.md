...
**Last updated:** 2026-08-24 15:38 CEST
...
- pushed `main` contains the `v1.2.9` release commit `e2612d2f01802527f5eb40569c88df57cb5c09dc` plus subsequent server-side Stripe testmode code commits through `916e845` (`feat(billing): record settlement transfer currency`) and the docs-only Stripe test-settlement close-out; the webserver has fast-forward deployed this state and `computemesh-gateway.service` is active on the `916e845` code path;
