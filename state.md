...
**Last updated:** 2026-08-24 11:45 CEST
...
**Production services/runtime:** no production inference/control/payment runtime. The public web/update server is reachable by SSH as `supersrv-trixie` and runs active `computemesh-autoupdate.service` plus `computemesh-gateway.service`. Gateway billing now has a fail-closed real Stripe Checkout/Webhook implementation path plus a Stripe Connect Accounts v2 provider onboarding/settlement execution path. The webserver is configured with Stripe **testmode** keys, a test webhook endpoint, a Stripe Accounts v2 thin-event destination, durable test ledger/session paths, and a durable SQLite account/settlement store. A real Stripe test Checkout for the laptop user completed and was credited through the signed public webhook path after the SDK-object normalization fix. Stripe Connect is activated in the `InetConnector Sandbox` account for marketplace testing, but provider onboarding/KYC, provider attribution to real jobs, live-mode Stripe credentials, and production database/ops controls are not configured end-to-end for real customer funds.
**Release/version truth:** signed update release `1.2.9` is committed, tagged, pushed, deployed to the live webserver, and installed on the reachable LAN miner. Windows/Linux artifacts were rebuilt for the signed release, NodeOS ISO/IMG manifest entries were preserved, the webserver manifest signature verifies, and `http://192.168.1.27:8080/api/status` reports `software.current_version = "1.2.9"`. `main` now contains additional server-side Stripe billing fixes after the `v1.2.9` app artifact tag; no newer signed client artifact release has been cut yet. `server-stripe-connect-v2-20260824` is a non-release deployment tag for the Stripe Connect Accounts v2 server activation. `v1.2.8` is the previous pushed signed release tag.
...
- pushed `main` contains the `v1.2.9` release commit `e2612d2f01802527f5eb40569c88df57cb5c09dc` plus subsequent server-side Stripe testmode commits through `da17b72` (`feat(billing): handle stripe accounts v2 events`); the webserver has fast-forward deployed this commit;
...
1. Complete the hCaptcha/phone step in the open Chrome Stripe onboarding tab for provider `node_test_settle_02` / `acct_1U7twhDbl73CGVC7`, finish the remaining Stripe test onboarding/KYC screens, refresh status until `payouts_enabled=true`, generate a real provider payable through metered usage, and run `POST /v1/admin/settlements/provider` in Stripe testmode.
