...
**Last updated:** 2026-08-24 10:00 CEST
...
2. **Webserver repo:** `/root/ComputeMesh` is clean on `main...origin/main` at `cd76382` after `git fetch --prune`, `git pull --ff-only origin main`, and Stripe integration deployment on 2026-08-24 09:58 CEST.
3. **Webserver services:** after redeployment at 2026-08-24 09:58 CEST, `computemesh-autoupdate.service` and `computemesh-gateway.service` are active/running. `systemctl show computemesh-autoupdate.service -p ExecStart` reports `/root/ComputeMesh/.venv/bin/python services/updater/auto_updater.py --daemon --interval 300 --version 1.2.9`. Gateway admin route `GET /v1/admin/server_status` reports `status: online`, `version: 1.2.9`, `git_commit: cd76382`, branch `main`.
...
11. **Live Stripe deployment status:** commit `cd76382` installed `stripe==15.5.1` into `/root/ComputeMesh/.venv` and restarted `computemesh-gateway.service`. The gateway unit currently has no `Environment=` entries for `STRIPE_API_KEY`, `STRIPE_WEBHOOK_SECRET`, `COMPUTEMESH_STRIPE_SESSION_STORE`, or `COMPUTEMESH_GATEWAY_LEDGER_PATH`; therefore live Checkout is intentionally fail-closed. Verified responses: `/v1/billing/checkout` with a bearer key returns HTTP 400 `STRIPE_API_KEY is required for Stripe Checkout`; `/v1/billing/webhook` without `Stripe-Signature` returns HTTP 400 `Missing Stripe-Signature header`; `/v1/billing/topup` with only a bearer key returns HTTP 403 admin authentication required.
