...
**Last updated:** 2026-08-24 10:45 CEST
...
- professional Stripe Connect provider-account/onboarding/settlement foundation: pending commit in this work block
...
1. Configure `COMPUTEMESH_ACCOUNT_STORE_PATH` on the live webserver, restart `computemesh-gateway.service`, and verify `/v1/admin/server_status` reports `account_store_configured=true`.
2. Create at least one Stripe Connect test provider through `/v1/providers/stripe/onboarding`, complete test onboarding in Stripe, refresh status, generate a real provider payable through metered usage, and run `POST /v1/admin/settlements/provider` in Stripe testmode.
