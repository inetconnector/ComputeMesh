...
**Last updated:** 2026-08-24 11:10 CEST
...
**Production services/runtime:** no production inference/control/payment runtime. The public web/update server is reachable by SSH as `supersrv-trixie` and runs active `computemesh-autoupdate.service` plus `computemesh-gateway.service`. Gateway billing now has a fail-closed real Stripe Checkout/Webhook implementation path plus a Stripe Connect Accounts v2 provider onboarding/settlement execution path. The webserver is configured with Stripe **testmode** keys, a test webhook endpoint, durable test ledger/session paths, and a durable SQLite account/settlement store. A real Stripe test Checkout for the laptop user completed and was credited through the signed public webhook path after the SDK-object normalization fix. Stripe Connect is activated in the `InetConnector Sandbox` account for marketplace testing, but provider onboarding/KYC, provider attribution to real jobs, live-mode Stripe credentials, production database/ops controls, and production Stripe event-destination coverage are not configured end-to-end for real customer funds.
...
- canonical merged **code baseline before this work block**: `31dc3a4` (`docs(state): record stripe connect refresh deployment`)
...
- pushed `main` contains the `v1.2.9` release commit `e2612d2f01802527f5eb40569c88df57cb5c09dc` plus subsequent server-side Stripe testmode commits through `31dc3a4`; the current worktree adds Accounts v2 support and must be pushed/deployed before the webserver can create new Stripe connected accounts;
...
9. Stripe Connect external-account activation is no longer blocked by missing Dashboard signup in testmode. The Stripe account now requires Accounts v2 for new connected accounts; the code path has been updated accordingly and must be deployed with `COMPUTEMESH_STRIPE_CONNECT_API=v2`.
...
1. Deploy the Accounts v2 Connect code, set `COMPUTEMESH_STRIPE_CONNECT_API=v2` on the webserver, then create at least one Stripe Connect test provider through `/v1/providers/stripe/onboarding`, complete test onboarding in Stripe, refresh status, generate a real provider payable through metered usage, and run `POST /v1/admin/settlements/provider` in Stripe testmode.
