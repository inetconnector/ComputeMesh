...
**Last updated:** 2026-08-24 11:14 CEST
...
- pushed `main` contains the `v1.2.9` release commit `e2612d2f01802527f5eb40569c88df57cb5c09dc` plus subsequent server-side Stripe testmode commits through `1afb048` (`feat(billing): use stripe connect accounts v2`); the webserver has fast-forward deployed this commit;
...
1. Complete the single-use Stripe test onboarding for provider `node_test_settle_02` / `acct_1U7twhDbl73CGVC7`, refresh status until `payouts_enabled=true`, generate a real provider payable through metered usage, and run `POST /v1/admin/settlements/provider` in Stripe testmode.
