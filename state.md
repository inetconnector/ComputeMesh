...
**Last updated:** 2026-08-24 10:12 CEST
...
2. **Webserver repo:** `/root/ComputeMesh` is clean on `main...origin/main` at `0698d5a` after `git fetch --prune`, `git pull --ff-only origin main`, and Stripe testmode deployment on 2026-08-24 10:10 CEST.
3. **Webserver services:** after redeployment at 2026-08-24 10:10 CEST, `computemesh-autoupdate.service` and `computemesh-gateway.service` are active/running. `systemctl show computemesh-autoupdate.service -p ExecStart` reports `/root/ComputeMesh/.venv/bin/python services/updater/auto_updater.py --daemon --interval 300 --version 1.2.9`. Gateway admin route `GET /v1/admin/server_status` reports `status: online`, `version: 1.2.9`, `git_commit: 0698d5a`, branch `main`.
...
11. **Stripe testmode deployment status:** commit `0698d5a` installed the Stripe integration that passes a configurable product tax code and restarted `computemesh-gateway.service`. `/root/ComputeMesh/.venv` has `stripe==15.5.1`. `/root/ComputeMesh/config/security/stripe.txt` was copied from `\\diskstation\Dani\ComputeMesh\stripe.txt` (SHA-256 `108b23bac6810c7af77391f42860c52a95071870896376a3bc3647ee683562d4`, 279 bytes) and parsed into `/root/ComputeMesh/config/security/stripe.env` with `0600` permissions. The env contains Stripe test `sk_test...`, test `pk_test...`, generated `whsec...`, `COMPUTEMESH_STRIPE_SESSION_STORE=/var/lib/computemesh/stripe_sessions.json`, `COMPUTEMESH_GATEWAY_LEDGER_PATH=/var/lib/computemesh/gateway_ledger.json`, and `COMPUTEMESH_STRIPE_PRODUCT_TAX_CODE=txcd_10105002`. Do not commit or print the secret values.
...
13. **Stripe test verification:** `POST /v1/billing/checkout` on `127.0.0.1:8000` with a test bearer key and `$5` returned HTTP 200 with a real Stripe `cs_test...` Checkout Session URL on `checkout.stripe.com`; `/var/lib/computemesh/stripe_sessions.json` now exists and records one created test session with `amount_micro_units=5000000`, `livemode=False`. `/v1/billing/webhook` without `Stripe-Signature` returns HTTP 400 `Missing Stripe-Signature header`; with an invalid signature returns HTTP 400 `No signatures found matching the expected signature for payload`.
...
4. Add durable account storage and wire portal registration tokens to gateway accounts instead of keeping both stores separately in memory.
5. Add provider enrollment that binds node identity, provider ledger account, payout address, and wallet ownership proof. For EVM addresses, require a signed challenge before accepting or changing payout targets.
...
7. Implement real payout execution: Stripe Connect/SEPA for bank payouts, optional Stripe-supported wallet/stablecoin payout where available, and a separate operator treasury payout executor. The operator's 25% must have an explicitly configured destination account before payout jobs can run.
