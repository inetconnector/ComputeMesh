...
$script:Text = @{
    de = @{
        Title='ComputeMesh Lab Setup'; Menu='Was möchtest du tun?'; Node='Diesen Rechner vorbereiten und Profil erstellen'; Server='Netzwerktest: dieser Rechner wartet (Server / Node B)'; Client='Netzwerktest: Verbindung zum anderen Rechner messen (Client / Node A)'; Llama='llama.cpp Prefill/Decode messen'; Tests='Alle lokalen Tests ausführen'; Exit='Beenden'; Choice='Auswahl'; Press='Enter drücken zum Fortfahren'; PythonMissing='Python 3.10+ fehlt. Installation über winget wird versucht.'; NoWinget='winget fehlt. Bitte Python 3.10+ installieren und SETUP.cmd erneut starten.'; Preparing='Lokale Python-Umgebung wird vorbereitet...'; Done='Fertig.'; Results='Ergebnisse'; Summary='Kurzergebnis'; TestsInstall='Test-Abhängigkeiten werden nur in .venv installiert.'; Profile='Profil'; Memory='RAM'; Rtt='RTT p50 / p95'; Throughput='Upload / Download'; Prefill='Prefill'; Decode='Decode'; Cancelled='Abgebrochen.'
    }
    en = @{
        Title='ComputeMesh Lab Setup'; Menu='What do you want to do?'; Node='Prepare this computer and capture its profile'; Server='Network test: wait here (Server / Node B)'; Client='Network test: measure the other computer (Client / Node A)'; Llama='Measure llama.cpp prefill/decode'; Tests='Run all local tests'; Exit='Exit'; Choice='Choice'; Press='Press Enter to continue'; PythonMissing='Python 3.10+ is missing. Setup will try winget.'; NoWinget='winget is unavailable. Install Python 3.10+ and run SETUP.cmd again.'; Preparing='Preparing local Python environment...'; Done='Done.'; Results='Results'; Summary='Summary'; TestsInstall='Test dependencies are installed only into .venv.'; Profile='Profile'; Memory='RAM'; Rtt='RTT p50 / p95'; Throughput='Upload / Download'; Prefill='Prefill'; Decode='Decode'; Cancelled='Cancelled.'
    }
}
