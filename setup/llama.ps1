...
function Install-OfficialLlama {
    Write-Host 'Downloading the latest official llama.cpp Windows release...'
    ...
    if (-not $mainAsset) { throw 'No suitable official Windows llama.cpp asset found.' }
...
function Invoke-LlamaSetup {
    ...
    if (-not $bench) {
        Write-Host '1. Download official llama.cpp automatically'; Write-Host '2. Choose existing llama-bench.exe'
        ...
        if (-not $bench) { $bench = Select-LocalFile 'llama-bench.exe|llama-bench.exe|Executables (*.exe)|*.exe' 'Choose llama-bench.exe' }
    }
    ...
    if (-not $model) { $model = Select-LocalFile 'GGUF model (*.gguf)|*.gguf|All files (*.*)|*.*' 'Choose GGUF model' }
